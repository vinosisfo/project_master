...
<table class="customers" style="white-space:nowrap;table-layout : auto;">
  <tr>
    ...
    <td>:</td>
    <td>
      <input type="text" name="jabatan_src" id="jabatan_src">
    </td>
...
<script type="text/javascript">
    ...
    function input_data(){
      $.post("<?php echo base_url('jabatan/c_jabatan/input_data')?>",{inpt : "input"},function(data){
    ...
    function edit_data(kode){
      $.post("<?php echo base_url('jabatan/c_jabatan/edit_data')?>",{kode : kode},function(data){
        $("#modal-default").modal("show")
        $("#data_detail").html(data)
      })
    }
    ...
    table = $('#tb_data_list').DataTable({ 
      ...
      "order"       : [],
      "ajax"        : {
          "url" : "<?php echo base_url('jabatan/c_jabatan/get_data')?>",
          "type": "POST",
          "data": function ( data ) {
              data.jabatan_src        = $('#jabatan_src').val()
