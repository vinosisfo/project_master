    ...
    <div class="table-responsive">
        <table class="table table-striped">
            <tr>
                ...
                <td>
                    <input type="text" name="max_id" id="max_id" class="transparant" readonly value="<?php echo $head->MsGroupId ?>">
                </td>
            </tr>
            <tr>
                <td>Nama Group</td>
                <td>:</td>
                <td>
                    <input value="<?php echo $head->MsGroupNama ?>" type="text" name="nama_group" id="nama_group" maxlength="200" style="max-width: 400px; text-transform:uppercase;">
                    <input value="<?php echo $head->MsGroupNama ?>" type="hidden" name="nama_group_old" id="nama_group_old" maxlength="200" style="max-width: 400px; text-transform:uppercase;">
                </td>
            </tr>
        </table>
        ...
        <div class="tableFixHead2">
            <table class="customers">
                ...
                foreach ($list_menu->result() as $data) {
                    $no++;
                    $checked     = ($data->MsMenu_ID==$data->ID_GROUP_MENU) ? "checked" : "";
                    $checked_val = ($data->MsMenu_ID==$data->ID_GROUP_MENU) ? ($data->MsMenu_ID) : ""; ?>
                    <tr>
                        <td><?php echo $no ?></td>
                        <td>
...
<script>
    function simpan_data(){
        ...
        nama_group_old = $("#nama_group_old").val();
        if(nama_group==""){
        ...
        $.post("<?php echo base_url()?>group_user/c_group_user/cek_nama_group",{nama_group : nama_group,nama_group_old : nama_group_old,edit : "edit"},function(data){
            if(data.pesan=="ada"){
    ...
    function simpan_data_nex(){
        ...
        $.post("<?php echo base_url()?>group_user/c_group_user/update_data",$("#form_group").serialize(),function(data){
            if(data.pesan=="ok"){
                table.ajax.reload();
